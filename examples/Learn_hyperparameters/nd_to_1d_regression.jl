...
function estimate_cost_covariance(
    ...
    for i in 1:n_samples
        costs[i] = calculate_cost(rng, l, s, noise_sd, n_features, batch_sizes, io_pairs, n_perm)
    end
...
for i in 1:N_iter
    ...
    g_ens = calculate_ensemble_cost(rng, lvec, μ_s, noise_sd, n_features, batch_sizes, io_pairs, n_perm = n_perm)
    ...
    println(
        ...
        string(err[i]) *
        ", for mean-parameters" *
        string(mean(transform_unconstrained_to_constrained(priors, get_u_final(ekiobj)), dims = 2)),
    )
end
